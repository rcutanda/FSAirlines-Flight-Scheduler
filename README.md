...
>**REQUIREMENTS:**
>
>**1. A PHP-enabled web server.** This should not be a problem, as most virtual airlines will host a website. Nevertheless, the script can also be run locally using XAMPP, WAMP, MAMP, or a similar PHP stack.
>
...
**4.2**. **Using the coordinates obtained from FSA**, the direct distance (great circle route) between origin and destination is calculated. After that, using the **speed, cruise altitude, and ascend/descend rate and speed** specified in the form, the **en route time is computed**. 
...
4.3.3 **Default climb and descent rates are 1,800 feet per minute** for flights with speed indicated in Mach, and 800 feet per minute for flights with speed indicated in knots.
